...
pub const Lms = struct {
    ...
    fn update(self: *Lms, sample: i32, residual: i32) void {
...
fn clamp_s16(v: i32) i32 {
    if (@as(usize, @intCast(v)) + 32768 > 65535) {
...
pub fn decode(bytes: []const u8) !struct { samples: []u16, desc: Description } {
    const desc = try decodeHeader(bytes);
    return .{ .samples = undefined, .desc = desc };
}
...
pub fn decodeHeader(bytes: []const u8) !Description {
    ...
    const samplerate: u32 = @intCast((frame_header >> 32) & 0xffffff);
